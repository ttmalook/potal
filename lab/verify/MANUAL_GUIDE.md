# 수동 검증 가이드 (사람이 직접 실행) — 50종

컨테이너에 직접 들어가 **명령어를 한 줄씩** 입력해서, ① 취약 환경을 만들고 → ② 취약함을 눈으로 확인하고 → ③ 조치 가이드대로 명령을 넣고 → ④ 조치됨을 확인하는 **실습 가이드**입니다. 자동 러너(`verify/verify.sh`)와 달리, 각 단계를 사람이 직접 실행합니다.
...
```
이 수동 가이드는 그 자동 검증이 실제로 무엇을 하는지 **사람이 한 줄씩 직접 확인**하기 위한 것입니다.
