...
//    🔨 to-build      : 표준 랩으로 재현 가능해 보이는데 아직 템플릿 없음 (= 만들 목록)
//    📄 guide-only    : 인프라 랩으로 재현 불가(유출/침해/패치지연 등) → 가이드만
//    ⚠️ stale         : 우리가 지원한다고 적었지만 SSC 카탈로그에 없는 key
...
function main(cat) {
  ...
  console.log(`🔨 재현 가능·미구축(만들 목록): ${toBuildCount}`)
  ...
  console.log(`📄 가이드-only(재현 불가) : ${buckets.guideOnly.length}`)
  if (stale.length) console.log(`⚠️ 우리 지원목록에만 있음(SSC 없음): ${stale.length} → ${stale.join(', ')}`)
  console.log('')
  console.log('─── 🔨 만들 목록 (재현 가능해 보이나 아직 템플릿 없음) ───')
  ...
  console.log('')
  console.log(`(📄 가이드-only ${buckets.guideOnly.length}건은 유출/침해/패치지연 등 인프라 랩으로 재현 불가 → --json 으로 전체 확인)`)
}
